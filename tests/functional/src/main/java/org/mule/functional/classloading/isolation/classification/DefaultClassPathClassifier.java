...
public class DefaultClassPathClassifier implements ClassPathClassifier {
   ...
   * for each artifact that contains the {@value #SERVICE_PROPERTIES_FILE_NAME} file, {@cvalue #SERVICE_PROVIDER_CLASS_NAME}
   * will be used as {@link ArtifactClassLoader#getArtifactName()}.
   * Once an artifact is identified the dependencies are going to be collected from the graph using Maven {@code provided} scope.
   *
   * @param extendedContext {@link ExtendedClassPathClassifierContext} that holds the data needed for classifying the artifacts
    ...
    try {
      ...
      for (Resource resource : resources) {
        ...
        if (!extendedContext.getClassificationContext().getServicesExclusion().contains(serviceMavenArtifactId)) {
          logger.debug("Service: '" + serviceProviderClassName + "' from artifactId: '" + serviceMavenArtifactId + "' found and being classified");
          ...
          logger.debug("Service: '" + serviceProviderClassName + "' from artifactId: '" + serviceMavenArtifactId + "' found and ignored as it is excluded");
        }
      }
    } catch (IOException e) {
